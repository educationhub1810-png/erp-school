...
export function StaffRowActions({ role, roleLabel, staff }: Props) {
  ...
  const deleteStaff = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error((json as { error?: string }).error || `Failed to delete ${roleLabel.toLowerCase()}`);
        return;
      }
      toast.success(`${staff.user.name} deleted`);
      router.refresh();
    } finally {
      setDeleting(false);
  ...
  return (
    <div className="flex items-center justify-end gap-1.5">
      ...
      <EditStaffDialog role={role} roleLabel={roleLabel} staff={staff} open={editOpen} onOpenChange={setEditOpen} />
    </div>
  );
}
