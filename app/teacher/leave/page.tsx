...
export default async function TeacherLeavePage() {
  ...
  const [myRequests, studentRequests] = await Promise.all([
    prisma.leaveRequest.findMany({
      where: { userId: user.id, schoolId: user.schoolId },
      orderBy: { createdAt: "desc" },
    }),
    prisma.leaveRequest.findMany({
      where: { schoolId: user.schoolId, user: { role: "STUDENT" } },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Apply for leave and review student leave requests</p>
        </div>
        <ApplyLeaveDialog />
      </div>
      ...
      <Card className="border-0 shadow-sm">
        ...
        <CardContent className="p-0">
          {myRequests.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-12">No leave applications yet.</p>
          ) : (
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <ClipboardCheck className="w-4 h-4" /> Student Leave Requests
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {studentRequests.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-12">No student leave requests.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-gray-50">
